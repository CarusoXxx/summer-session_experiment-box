...
/* ── 中断开关：NFC 操作期间临时关闭 UART1 中断，防止 FPR ISR 抢数据 ── */
static void NFC_IRQ_Off(void) { USART_ITConfig(USART1, USART_IT_RXNE, DISABLE); }
static void NFC_IRQ_On(void)  { USART_ITConfig(USART1, USART_IT_RXNE, ENABLE);  }

/* ── 轮询接收 + 超时等待 ── */
static int8_t NFC_WaitRx(uint32_t timeout_ms) {
    uint32_t t = 0;
    PN532_RxBufLen = 0;
    while (t < timeout_ms) {
        while (USART_GetFlagStatus(USART1, USART_FLAG_RXNE) != RESET) {
            if (PN532_RxBufLen < sizeof(PN532_RxBuf))
                PN532_RxBuf[PN532_RxBufLen++] = USART_ReceiveData(USART1);
        }
        if (PN532_RxBufLen > 0) return 0;
        delay_ms(1); t++;
    }
    return -1;  /* 超时 */
}
...
void NFC_Init(uint32_t baud ){
#if 0  /* GPIO/UART init done by FPR module. NFC only re-inits baud rate */
    ...
    /* ── 重新配置 UART1 波特率（GPIO 和外设时钟由 FPR_Init 已配置）── */
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART1, ENABLE);
    {
        USART_InitTypeDef s;
        USART_StructInit(&s);
        s.USART_BaudRate = baud;
...
/*
UART1中断处理函数 — 与FPR共用，需跳线切换。编译时可选。
*/
#if 0  /* 使用 NFC 时打开此宏，同时注释 fpr_zn632.c 中的同名函数 */
...
#endif /* #if 0 — NFC ISR disabled, using FPR ISR instead */


/* ── NFC 模式轮询接收（ISR 被禁用时的替代）── */
void PN532_PollRx(void) {
    while (USART_GetFlagStatus(USART1, USART_FLAG_RXNE) != RESET) {
        if (PN532_RxBufLen < sizeof(PN532_RxBuf)) {
            PN532_RxBuf[PN532_RxBufLen++] = USART_ReceiveData(USART1);
        }
